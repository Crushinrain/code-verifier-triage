...
def test_active_contract_freeze_drift_fails_through_real_cli(
    ...
    result = run_cli(DIGEST_CHECKER, root)
    output = result.stdout + result.stderr
    assert result.returncode != 0, output
    assert "active contract digest mismatch for contracts/project.yaml" in output
    assert f"expected {expected}, got {actual}" in output
